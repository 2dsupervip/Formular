...
# ==========================================
st.set_page_config(page_title="2D AI Master V35.6 Master-Lab", layout="centered", page_icon="🤖")
...
st.markdown('<div class="main-title">🤖 THE PERFECT 2D AI MASTER (V35.6)</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">Advanced Pair-Engine Lab | Custom n% to 100% Rate Threshold Filter</div>', unsafe_allow_html=True)
...
if 't_day' not in st.session_state: st.session_state.t_day = ""
if 't_time' not in st.session_state: st.session_state.t_time = ""
...
# ==========================================
# MASTER ROUTINE: V35.6 INTERSECTION ENGINE
# ==========================================
    ...
    for mu_k in mu_keys_list:
        ...
        for cand_val in cand_list:
            ...
            card_payload = {
                ...
                "success_hits": successful_hits, "total_hits": total_recent_count,
                "is_deadline": is_deadline_flag, "pure": last_generated_val, "mu_k": mu_k, 
                "advisor": sniper_note, "rate": rate, "max_span": max_required_span, 
                "lbl_prefix": lbl_prefix, "label_space": label_space
            }
            ...
            if not is_research_mode and rem_steps in [2, 3]:
                ...
                recovery_score = base_quality_score * (1.5 if rem_steps == 2 else 1.0) * (100.0 / coverage_count)
                recovery_pool.append({"key": last_generated_val, "lbl_prefix": lbl_prefix, "rem_steps": rem_steps, "score": round(recovery_score, 1), "card": card_payload})

    return step_buckets, recovery_pool

# ==========================================
# FILE UPLOAD & LOAD DATA TOOL
# ==========================================
def load_and_process_data(file_bytes, file_name):
    try:
        ...
        if not all(col in df.columns for col in ['year', 'day', 'am1', 'am2', 'pm1', 'pm2']):
            return "⚠️ ဖိုင်ထဲတွင် လိုအပ်သော ကော်လံများ (year, day, am1, am2, pm1, pm2) မပြည့်စုံပါ!"
        ...
        last_draw = full_draws[-1]
        st.session_state.t_day = last_draw['day'] if last_draw['time'] == 'AM' else active_days_cycle[(active_days_cycle.index(last_draw['day']) + 1) % len(active_days_cycle)]
        st.session_state.t_time = "PM" if last_draw['time'] == 'AM' else "AM"
        st.session_state.full_draws = full_draws
        st.session_state.active_days = active_days_cycle
        
        # Build Day Pairs Mapping for Advanced Pair Engine
        day_pairs = {}
        for d in full_draws:
            r = d['row_idx']
        ...
        return f"⚠️ Data Process အမှားအယွင်း: {e}"
...
if st.session_state.full_draws:
    # ✅ [MODERN SUCCESS SIGNATURE BLOCK]
    ...
    # ------------------------------------------
    # TAB 1: LIVE AUTO TRACKER (FORM PROTECTED)
    # ------------------------------------------
    with tab_live:
    ...
    # ------------------------------------------
    # TAB 2: ADVANCED PAIR-ENGINE LAB (FORM PROTECTED)
    # ------------------------------------------
    with tab_custom:
        ...
        with st.form("custom_research_form"):
            ...
            with c2: trigger_num = st.text_input("🔍 အစပြုဂဏန်း/Keyword:", value="30 အလယ်")
            with c3: custom_max_tf = st.number_input("⏳ စစ်ဆေးမည့်ပွဲစဉ်:", min_value=1, max_value=40, value=20)
            
            # 📈 [DYNAMIC RATE THRESHOLD INPUT BOX]
            min_rate_param = st.number_input("📈 အနည်းဆုံး ကိုက်ညီမှုနှုန်း (n% မှ 100% နှုန်းသတ်မှတ်ရန်):", min_value=1, max_value=100, value=90, step=5)
            
            search_space = st.selectbox("🔍 မူများရှာဖွေမည့် Space:", ["All", "AM+PM ပေါင်းချုပ်", "AM သီးသန့်", "PM သီးသန့်"])
        ...
        if submit_custom:
            ...
            # --- 🚀 ADVANCED PAIR ENGINE KEYWORD PARSING ---
            if "အလယ်" in clean_trigger:
                    ...
                    for r, pair in st.session_state.day_pairs.items():
                        if pair['AM'] and pair['PM']:
                            ...
                            if mid_pair == digits or mid_pair == digits[::-1]:
                                target_hits.append(pair['PM']) # Trigger target established at PM
                ...
                # Standard Exact/Reverse digit scanning fallback
                digits = "".join(re.findall(r'\d+', clean_trigger))
                if digits:
